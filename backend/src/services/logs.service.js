 ...
 * ============================================
 */
const getSeverity = (data, score, attackType) => {
  if (attackType === "DDoS" || data.requests > 2000 || score >= 0.85) {
    return "critical";
  }

  if (attackType === "Brute Force" || data.failedLogins > 20 || score >= 0.65) {
    return "high";
  }
 ...
 * ============================================
 */
const processLog = async (logData, io, userId) => {
  try {
    ...
    try {
      ...
      logger.error(`ML error: ${error.message}`);
    }
     ...
     * ============================================
     */
    let attackType = normalizeAttackType(prediction.attackType);
      ...
      alert = await createAlert(
        {
          ...
          message: `${attackType} threat detected`,
    ...
    logger.info(`Log processed for ${userId}`);
    ...
    logger.error(`processLog error: ${error.message}`);
...
const getLogs = async (userId, options = {}) => {
  try {
    ...
    if (options.attackType) {
      query.attackType = options.attackType;
    }

    if (typeof options.is_anomaly !== "undefined") {
      query.is_anomaly = options.is_anomaly;
    }

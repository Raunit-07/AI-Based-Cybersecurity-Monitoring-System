...
export const AlertsList = ({ alerts, limit }) => {
  ...
  const getSeverityIcon = (severity) => {
    switch (severity.toLowerCase()) {
      ...
      default: return <Shield className="w-5 h-5 text-success" />;
    }
  };
        ...
        {(!displayAlerts || displayAlerts.length === 0) ? (
          ...
          <div className="space-y-3">
            {displayAlerts.map((alert) => (
              <div 
                key={alert.id} 
                className={`p-4 rounded-lg border flex items-start gap-4 transition-all hover:scale-[1.01] ${getSeverityColor(alert.severity)}`}
              >
                <div className="mt-1">
